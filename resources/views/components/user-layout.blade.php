<x-app-sidebar-layout>
    ...
    <x-slot name="sidebar">
        ...
        <li>
            @php
                $statUrl = $litProject ? route('statistics.index', $litProject) : route('user.projects.index');
            @endphp
            <a href="{{ $statUrl }}" class="group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold {{ request()->routeIs('statistics.*') ? 'bg-gray-50 text-fuchsia-600' : 'text-gray-700 hover:bg-gray-50 hover:text-fuchsia-600' }}">
                <svg class="h-6 w-6 shrink-0 {{ request()->routeIs('statistics.*') ? 'text-fuchsia-600' : 'text-gray-400 group-hover:text-fuchsia-600' }}" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
